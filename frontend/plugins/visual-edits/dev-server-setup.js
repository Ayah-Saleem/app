      ...
      try {
        ...
        Object.entries(changesByFile).forEach(([fileName, fileChanges]) => {
          ...
          try {
            ...
            execSync(`git -c user.name="visual-editor" -c user.email="admin@app.local" add "${targetFile}"`);
            execSync(`git -c user.name="visual-editor" -c user.email="admin@app.local" commit -m "visual_edit_${timestamp}"`);
          } catch (gitError) {
